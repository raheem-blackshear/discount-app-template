...
export class AccountEndpoint extends EndpointFactory {
    ...
    private readonly _contactUrl: string = "/api/account/contact";
    ...
    getSubmitContactEndpoint<T>(userObject: any): Observable<T> {

        return this.http.post<T>(this.contactUrl, JSON.stringify(userObject), this.getRequestHeaders())
            .catch(error => {
                return this.handleError(error, () => this.getSubmitContactEndpoint(userObject));
            });
    }
}
